workspace "Orion Relay Product System" "Product model for resilient remote store-and-forward communications." {
  model {
    sys_atlas_orion_relay = softwareSystem "Orion Relay Product System" "Product model for resilient remote store-and-forward communications." {
      ...
      group "Relay Mission" {
        fu_fu_ori_link = container "Link Scheduler Software" "Selects available radio links and transmission windows." "Functional Unit" {
          tags "FunctionalUnit"
        }
        fu_fu_ori_router = container "Message Router Software" "Stores, prioritizes, and forwards operational messages." "Functional Unit" {
          tags "FunctionalUnit"
        }
      }
    ...
    person_act_ori_controller = person "Network Controller" "Sets delivery priorities and monitors link state." {
      tags "Actor"
    }
    person_act_ori_engineer = person "Relay Security Engineer" "Owns product security risk." {
      tags "Actor"
    }
    ...
    group_fg_ori = softwareSystem "Relay Mission" "Product-specific link scheduling and delivery." {
    ...
    if_if_ori_message = softwareSystem "Relay Message Interface" "quic quic://relay/messages" {
      tags "Interface"
    }
    ...
    data_do_ori_message = softwareSystem "Relay Message" "" {
    ...
    ctrl_ctrl_ori_admission = softwareSystem "Priority Admission Control" "Reserves capacity for authenticated priority messages." {
      tags "Control,availability"
    }
    av_av_ori_message_flood = softwareSystem "Message Flooding" "Excess traffic attempts to starve critical relay messages." {
      tags "AttackVector"
    }
  ...
  views {
    ...
    systemContext sys_atlas_orion_relay "context" {
      include *
      autolayout lr
    }

    container sys_atlas_orion_relay "containers" {
      include *
      autolayout lr
    }
    dynamic sys_atlas_orion_relay "dynamic_compliance_flow_comp_employee_lifecycle" "Company workflow from pre-employment screening through onboarding, role changes, and offboarding." {
      person_compliance_act_people_operations -> fu_compliance_fu_comp_people "Employee Security Lifecycle" "Model flow: business"
      autolayout lr
    }
    styles {
