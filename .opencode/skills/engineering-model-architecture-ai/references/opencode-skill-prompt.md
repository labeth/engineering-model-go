...
- changed stable IDs
- changed source refs
- verification or coverage changes

Execution protocol:

- If task starts from business intent, map it to a `REQ-*` first.
- Use MCP lookup results for the shortest reasoning chain.
...
- If verification is missing, add/extend tests with `REQ-*` markers.

Done when:

- all modified requirement paths resolve through FU -> evidence -> verification,
- maintained generated artifacts are updated,
- tests pass,
